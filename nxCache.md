...
重定时（Retiming）控制缓存回放时，当前帧如何映射到缓存里的时间。

可用模式包括：
- Disabled：不做重定时，按正常缓存时间回放
- Frame：直接指定要播放的缓存帧
- Time(s)：直接指定要播放的缓存时间
- Custom：使用下方的 Timing 曲线自定义时间映射

它只影响回放阶段，不改变已经写进缓存的数据本身。

### 帧（Frame）

帧（Frame）只在重定时（Retiming）设为 Frame 时显示。

它表示当前要读取缓存里的哪一帧。给这个值打关键帧，就可以让缓存回放加速、减速、停顿，或者跳到指定缓存帧。

### 时间（Time(s)）

时间（Time(s)）只在重定时（Retiming）设为 Time(s) 时显示。

它表示当前要读取缓存里的哪个缓存时间。和帧（Frame）模式相比，这一项更适合直接按秒控制回放节奏。
...
颜色通道（Color Channel）控制体素缓存是否包含颜色数据。
...
点击后，插件会先同步当前场景和缓存设置。如果缓存目录已有内容，当前版本可以选择覆盖已有缓存，或者保留已有缓存继续写入。确认后会打开缓存构建进度窗口。
