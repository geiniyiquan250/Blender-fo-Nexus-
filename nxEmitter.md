...
当前插件已经支持把网格（Mesh）、曲线（Curve）和曲线对象（Curves）作为物体发射源；但不同来源类型能用到的筛选方式和发射来源并不完全一样，要以当前对象实际显示出来的参数为准。

### 选择来源（Selection）

选择来源（Selection）控制物体发射时，用哪种数据来决定哪些位置更容易发射。

当前有两种主要来源：
- Vertex Group：用顶点组权重控制
- Attribute：用点浮点属性控制

它本身不直接决定从点、边、面还是体积发射，而是决定“哪些位置参与发射、哪些位置被压低或排除”。

### 属性（Attribute）

属性（Attribute）只在选择来源（Selection）设为 Attribute 时显示。

它用于指定一个点浮点属性，让该属性的数值直接作为发射权重来源。数值越高的位置越容易通过阈值（Threshold）筛选并参与发射。

### 顶点组（Vertex Group）

只在选择来源（Selection）设为 Vertex Group 时使用，通常对象也需要提供可用的顶点组数据。
...
- 体积（Volume）：从物体内部体积里分布出生点
- 纹理（Texture）：和贴图或权重分布有关

它只在形状（Shape）是物体（Object）时才有意义。不同对象类型能否完整支持这些模式，要看当前对象是否能提供对应的点、边、面或体积数据。

### 每元素一个（One per Element）

每元素一个（One per Element）只在发射来源（Emit From）为点（Points）、边（Edges）或多边形中心（Polygon Center）时出现。

开启后，每个可用元素只生成一个粒子，发射结果会更规整，也更容易和原始几何元素一一对应。它适合做按点发射、按边发射，或者需要稳定元素编号关系的场景。

关闭后，同一个元素上仍然可以按当前发射逻辑分布多个出生点。想让发射更密或更随机时，可以保持关闭。

### 阈值（Threshold）

这个参数会作为物体发射里的一个筛选阈值传到底层逻辑里。
结合顶点组（Vertex Group）或属性（Attribute）去看，它更像一个“低于多少就不发”的门槛。
...
如果你想让脉冲发射晚一点启动，或者和其他修改器、缓存、动画对齐，先调这里。
...
当前版本里，它表示每秒或每帧产生多少粒子，具体取决于界面当前的时间/速率切换状态。
...
这一页用于让新出生的粒子继承发射来源在运动中的速度信息。

它适合用在移动中的物体（Object）、旋转中的发射器，或任何你希望粒子出生时就带着来源运动趋势的情况。打开后，粒子会更容易跟上来源的整体动势。

### 使用运动继承（Use Motion Inheritance）

使用运动继承（Use Motion Inheritance）控制是否启用这套继承计算。

开启后，下面的强度（Strength）、线性方向（Linear Direction）和旋转方向（Rotational Direction）才会参与生效。关闭后，粒子仍然只使用发射器本身设置的初始速度（Speed）。

### 强度（Strength）

强度（Strength）控制继承到粒子身上的运动量有多大。

数值越高，粒子出生时越接近直接带上来源的运动速度；数值越低，继承效果越轻。它适合拿来平衡“保留来源动势”和“保留自身喷射方向”之间的比例。

### 线性方向（Linear Direction）

线性方向（Linear Direction）控制是否继承来源的平移运动。

如果发射来源本身正在直线移动、跟随路径、被父级带动，开启它后，粒子出生时会更自然地带着这部分移动趋势。

### 旋转方向（Rotational Direction）

旋转方向（Rotational Direction）控制是否继承来源的旋转运动。

当发射来源正在绕轴旋转，或发射点分布在一个持续转动的对象表面上时，开启它可以让粒子更明显地带上甩出的旋转动势。
